...
# Main Plot Area - Adjusted to close the vertical title gap
ax = fig.add_axes([0.04, 0.18, 0.93, 0.71]); ax.set_xlim(-10, 10.6); ax.set_ylim(1.1, 12.0)

# Background structural bands
ax.axhspan(7.7, 10.8, color=PINK, zorder=0)
ax.axhspan(1.5, 6.6, color=BLUE, zorder=0)

# Solid parity baseline axis
ax.axvline(0, color="#333", lw=1.5, zorder=4)

# Helper function to generate perfect square inset axes for perfectly round icons
def sq_axes(x, y, w=0.035):
    a = fig.add_axes([x, y, w, w*FW/FH])
    a.axis("off")
    a.set_xlim(0, 1)
    a.set_ylim(0, 1)
    a.set_aspect("equal")
...
def target(a, c):
    ...
    for r, col in [(0.34, c), (0.24, "white"), (0.13, c)]: 
        a.add_patch(Circle((0.5, 0.5), r, color=col))

# Render data bars and labels
...
# Raised group headers outside of background bands using explicit figure layouts
factory(sq_axes(0.04, 0.852, w=0.035), RED)
fig.text(0.082, 0.859, "CHINA LEADS THE INPUTS", fontsize=15, fontweight="bold", color=RED, va="center")
fig.text(0.04, 0.832, "the raw ingredients: people, money, factories", fontsize=11, style="italic", color="#777")

rocket(sq_axes(0.04, 0.565, w=0.035), NAVY)
fig.text(0.082, 0.572, "AMERICA BUILDS THE COMPANIES", fontsize=15, fontweight="bold", color=NAVY, va="center")
fig.text(0.04, 0.555, "turning those inputs into fast-growing, profitable firms", fontsize=11, style="italic", color="#777", va="top")

# Callout text annotations (Pushed corporate profits callout safely to x=5.0)
...
ax.text(5.0, 2.0, "\u2190 driven by a home market\n   1.7\u00d7 larger (consumer\n   spending: 68% vs 40% of GDP)",
        fontsize=10.5, color=NAVY, style="italic", va="center", zorder=5)

# Bottom layout axis configurations
ax.set_yticks([])
ax.set_xticks([-9,-6,-3,0,3,6,9])
ax.set_xticklabels(["9\u00d7","6\u00d7","3\u00d7","","3\u00d7","6\u00d7","9\u00d7"], fontsize=11)
ax.tick_params(length=0)
for s in ["top","right","left"]: ax.spines[s].set_visible(False)
ax.spines["bottom"].set_color("#cccccc")

# Perfectly balanced directional labels inside the white channel space
fig.text(0.265, 0.152, "\u25c0  China ahead", fontsize=12.5, fontweight="bold", color=RED, ha="center")
fig.text(0.73, 0.152, "America ahead  \u25b6", fontsize=12.5, fontweight="bold", color=NAVY, ha="center")
...
# ---- takeaway lower container block (Shaded lightly with #f4f7fc for premium accent) ----
axT = fig.add_axes([0.04, 0.05, 0.93, 0.10]); axT.axis("off"); axT.set_xlim(0,1); axT.set_ylim(0,1)
axT.add_patch(FancyBboxPatch((0.005,0.06),0.99,0.88, boxstyle="round,pad=0.01,rounding_size=0.04",
                             fc="#f4f7fc", ec="#cfd3da", lw=1.3, mutation_aspect=0.20))
target(sq_axes(0.052, 0.082, w=0.035), NAVY)
